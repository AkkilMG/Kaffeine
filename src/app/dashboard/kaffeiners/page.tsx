...
import { useKaffeiners, useAdminKaffeiners } from '@/hooks/use-data';
...
export default function KaffeinersPage() {
  ...
  if (user?.role === 'admin') {
    return <AdminKaffeinersView />;
  }

  return <UserKaffeinersView />;
}

function AdminKaffeinersView() {
  ...
  const activeCount = kaffeiners.filter((k: any) => k.active).length;
